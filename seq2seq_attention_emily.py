...
class AttnDecoderRNN(nn.Module):
    ...
    def __init__(self, hidden_size, output_size, attn_type, num_head):
        ...
        self.attn_combine = nn.Linear(self.hidden_size * 2, self.hidden_size) #output of the hidden layer and context vector is of the same size, thus times 2, creating a matrix here
    ...
    def forward(self, input, output_enc, hidden_dec):
        ...
        attn_weights = F.softmax(self.attn(torch.cat((embedded[0], hidden_dec[0]),1)), dim = 1)
        attn_applied = torch.bmm(attn_weights.unsqueeze(0), output_enc.unsqueeze(0))
        # you need to consider the context vector passed from the attn function
        probs = torch.cat((embedded[0], attn_applied[0]),1)
        probs = self.attn_combine(probs).unsqueeze(0)
        probs = F.relu(output)
        probs, hidden_dec = self.gru(probs, hidden_dec)
        probs = F.log_softmax(self.out(probs[0]), dim = 1)
